...
def add_megatron_arguments(parser: argparse.ArgumentParser):
    ...
    parser = _add_benchmark_args(parser)   # 新增
...
def core_transformer_config_from_args(args, config_class=None):
    ...
    if args.init_method_kaiming_normal:
        kw_args['init_method'] = init_method_kaiming_normal()
        kw_args['output_layer_init_method'] = init_method_kaiming_normal()
        emb_std = (
            args.embedding_init_method_std
            if args.embedding_init_method_std is not None
            else args.init_method_std
        )
        kw_args['embedding_init_method'] = init_method_normal(emb_std)
    elif args.init_method_xavier_uniform:
...
def _add_benchmark_args(parser):
    """Add benchmark evaluation arguments (downstream eval during training)."""
    group = parser.add_argument_group(title='benchmark evaluation')
    ...
    group.add_argument('--benchmark-micro-batch', type=int, default=None,
                       help='Maximum number of sequences per forward pass for benchmark evaluation. If OOM, reduce this. Defaults to micro_batch_size.')
    group.add_argument('--benchmark-global-batch', type=int, default=None,
                       help='Total number of samples to evaluate per task (for quick validation). If None, evaluates all samples.')
    group.add_argument('--benchmark-root', type=str, default=None,
                       help='Root directory for benchmark data (e.g. Megatron-LM-evaluation/benchmark). If not set, uses repo_root/benchmark.')
    return parser
...
def _add_network_size_args(parser):
    ...
    group.add_argument('--normalization', default='LayerNorm',
                       choices=['LayerNorm', 'RMSNorm', 'ScaleOnly', 'NoNorm'],
...
def _add_regularization_args(parser):
    ...
    group.add_argument('--min-singular-reg-interval', type=int, default=0,
                       help='If > 0, every N steps add min singular value rank-1 term to 2D weights before weight decay. 0 disables.')
    group.add_argument('--min-singular-reg-scale', type=float, default=1.0,
                       help='Scale for min-singular regularization: W += scale * sigma_min * u_min * v_min^T.')          
    ...
    group.add_argument('--num-ns-steps', type=int, dest='muon_num_ns_steps',
                       help='Alias for --muon-num-ns-steps, used by Pion msign variants.')
    group.add_argument('--muon-coefficient-type', type=str, default='quintic',
                       choices=['simple', 'quintic', 'polar_express', 'aol', 'custom'],
                       help='Newton-Schulz coefficient type for Muon/Pion msign optimizers.')
    group.add_argument('--coefficient-type', type=str, dest='muon_coefficient_type',
                       choices=['simple', 'quintic', 'polar_express', 'aol', 'custom'],
                       help='Alias for --muon-coefficient-type, used by Pion msign variants.')
...
def _add_training_args(parser):
    ...
    group.add_argument('--optimizer', type=str, default='adam',
                       choices=['adam', 'sgd', 'muon', 'dist_muon', 'pion', 'pion_msign'],
                       help='Optimizer function')
    group.add_argument('--pion-spectrum-reset-interval', type=int, default=0,
                       help='Interval for resetting the spectrum of the parameters in Pion (only for --optimizer pion).')
    group.add_argument('--pion-scaling', type=str, default='rms',
                       choices=['rms', 'fnorm', 'brb', 'brb_rms', 'spectral', 'none'],
                       help='Scaling mode for Pion biside updates (only for --optimizer pion).')
    group.add_argument(
        '--pion-in-out-cos-csv',
        type=str,
        default=None,
        help='Optional CSV path: each Pion optimizer step logs mean cosine similarity between '
        'flattened (W @ A_in) and (A_out @ W), plus mean Frobenius norms of those two matrices, '
        'over all matrix update units (all-reduced over ranks).',
    )
    group.add_argument(
        '--pion-in-out-cos-csv-interval',
        type=int,
        default=1,
        help='Write interval for --pion-in-out-cos-csv (every N Pion optimizer steps).',
    )
    group.add_argument('--pion-degree', type=int, default=2,
                   help='Degree for Pion truncated matrix exponential (only for --optimizer pion).')
    group.add_argument('--pion-beta1', type=float, default=0.9,
                   help='First momentum coefficient for Pion.')
    group.add_argument('--pion-beta2', type=float, default=0.999,
                   help='Second momentum coefficient for Pion.')
    group.add_argument('--pion-lr', type=float, default=None,
                   help='Independent max learning rate for Pion matrix parameters. '
                        'If unset, Pion uses --lr.')
    group.add_argument('--pion-min-lr', type=float, default=None,
                   help='Independent min learning rate for Pion matrix parameters. '
                        'If unset, Pion uses --min-lr.')
    group.add_argument('--pion-rms', type=float, default=0.2,
                   help='RMS coefficient for Pion update scaling when --pion-scaling rms.')
    group.add_argument('--pion-msign-lambda', type=float, default=1.0,
                   help='Step-size multiplier for Pion msign orthogonal exponential updates '
                        '(only for --optimizer pion_msign).')
    group.add_argument('--pion-update-side', type=str, default='both',
                   choices=['both', 'alternate'],
                   help='Pion matrix update schedule. both uses bilateral W exp(A_in) and exp(A_out) W '
                        'every step; alternate updates only one side per step, switching odd steps to '
                        'W exp(A_in) and even steps to exp(A_out) W.')
    group.add_argument('--pion-no-split-qkv', action='store_false', default=True,
                   dest='pion_split_qkv',
    ...
    group.add_argument('--pion-qkv-split-granularity', type=str, default=None,
                       choices=['head', 'qkv', 'group'],
                       help='Attention QKV granularity for Pion. head: Q per attention head and K/V per GQA group; '
                            'qkv: only split the whole linear_qkv weight into Q, K, V; '
                            'group: split Q/K/V inside each GQA group. Defaults to head, or qkv when '
                            '--pion-no-split-qkv-per-head is used.')
    group.add_argument(
        '--pion-first-momentum',
        type=str,
        default='none',
        choices=['none', 'lie', 'ambient', 'ambient_transport'],
        help='Compatibility selector for Pion first-momentum geometry. In unified pion.py, '
        'first momentum is always used; this only selects Lie vs transported ambient when --pion-momentum is unset.',
    )
    group.add_argument(
        '--pion-second-momentum',
        type=str,
        default='none',
        choices=['none', 'lie', 'ambient', 'ambient_transport'],
        help='Compatibility selector for second-moment normalization. none disables it; any other value enables it.',
    )
    group.add_argument(
        '--pion-12-momentum',
        type=str,
        default='none',
        choices=['none', 'lie_lie', 'transported_ambient_ambient', 'ambient_ambient_transport'],
        help='Pion joint 1st+2nd order modes: lie_lie or transported_ambient_ambient '
        '(ambient_ambient_transport is accepted as an alias).',
    )
    group.add_argument(
        '--pion-momentum',
        type=str,
        ...
        choices=[
            ...
            'lie_lie',
            'transported_ambient',
            'transported_ambient_ambient',
            'ambient_ambient_transport',
        ],
        help='Unified Pion momentum geometry selector. Second-moment normalization is off by default '
        'and only enabled with --pion-use-second-momentum or legacy --pion-second-momentum/--pion-12-momentum.',
    )
    group.add_argument(
        '--pion-use-second-momentum',
        action='store_true',
        default=None,
        help='Enable second-moment normalization inside the selected Pion momentum geometry.',
    )
    group.add_argument(
        '--pion-exp-map',
        type=str,
        default='exp_truncated',
        choices=['exp_truncated', 'taylor'],
        help='Matrix map for Pion updates. exp_truncated uses the truncated Taylor expansion; '
        'taylor is kept as an alias.',
    )
    group.add_argument('--pion-update-csv', type=str, default=None,
                       help='CSV path to log Pion update stats (only for --optimizer pion).')
    group.add_argument('--pion-update-csv-interval', type=int, default=1,
                       help='Log Pion update stats every this many steps (only for --optimizer pion).')
...
def _add_initialization_args(parser):
    ...
    group.add_argument('--init-method-kaiming-normal', action='store_true',
                       help='Enable Kaiming normal initialization for linear weight '
                       'matrices. Embedding layers keep normal init via '
                       '--init-method-std (or --embedding-init-method-std).')
...
def _add_checkpointing_args(parser):
    ...
    group.add_argument('--save-initial-checkpoint', action='store_true', help='If set, save a checkpoint at iteration 0 (initialized model, before any training step).')
...
def _add_mixed_precision_args(parser):
    ...
    group.add_argument('--pure-bf16-optimizer', action='store_true',
                       help='Keep optimizer main params/states in bf16 (requires --bf16).')
...
def _add_validation_args(parser):
    ...
    group.add_argument(
        '--eval-top1-accuracy',
        action='store_true',
        help='If set, compute top-1 token accuracy during validation instead of lm loss'
        )
