...
from megatron.training.benchmark import run_benchmark
...
def preprocess_common_state_dict(common_state_dict):
    ...
    # # Bug fix: 在 deepcopy 之前，先创建一个浅拷贝并移除不可序列化的对象
    # # 这样可以避免在 deepcopy 时遇到 DataLoader 迭代器等不可序列化对象
    # cleaned_state_dict = {}
    # for key, value in common_state_dict.items():
    #     if key == 'args':
    #         # args 需要特殊处理：先转换为字典，移除不可序列化的对象
    #         if hasattr(value, '__dict__'):
    #             args_dict = {}
    #             for k, v in vars(value).items():
    #                 # 跳过 DataLoader 迭代器等不可序列化的对象
    #                 if 'iterator' in k.lower() or 'dataloader' in k.lower():
    #                     continue
    #                 # 跳过其他可能不可序列化的对象
    #                 try:
    #                     copy.deepcopy(v)
    ...
    #                     # 如果无法深拷贝，跳过这个键
    #                     continue
    #             cleaned_state_dict[key] = type(value)(**args_dict) if hasattr(type(value), '__init__') else args_dict
    #         else:
    ...
    # Bug fix: 在 deepcopy 之前，先创建一个浅拷贝并移除不可序列化的对象
    # 这样可以避免在 deepcopy 时遇到 DataLoader 迭代器等不可序列化对象
    # 特别处理 eval 后 save checkpoint 时 rerun_state_machine 可能包含 valid_data_iterator 引用的情况
    cleaned_state_dict = {}
    for key, value in common_state_dict.items():
        if key == 'args':
            # args 需要特殊处理：先转换为字典，移除不可序列化的对象
            if hasattr(value, '__dict__'):
                ...
                cleaned_state_dict[key] = value
        elif key == 'rerun_state_machine':
            # Bug fix: rerun_state_machine 需要特殊处理
            # 在 eval 后 save checkpoint 时，rerun_state_machine 可能包含 valid_data_iterator 的引用
            # 这些引用可能包含不可序列化的 DataLoader 迭代器
            if value is not None:
                if isinstance(value, dict):
                    # 如果 rerun_state_machine 是字典，清理其中的不可序列化对象
                    cleaned_rerun = {}
                    for k, v in value.items():
                        try:
                            ...
                            # 如果无法深拷贝，跳过这个键
                            # 特别是 data_iterator_checkpoints 可能包含不可序列化的对象
                            continue
                    cleaned_state_dict[key] = cleaned_rerun
                else:
                    # 如果不是字典，尝试直接复制（可能是 ShardedObject）
                    try:
                        copy.deepcopy(value)
                        cleaned_state_dict[key] = value
                    except (TypeError, NotImplementedError):
                        # 如果无法深拷贝，设置为 None（rerun_state_machine 在 eval 后可能不需要保存）
                        cleaned_state_dict[key] = None
            else:
                cleaned_state_dict[key] = value
        else:
            # 其他键直接复制
    ...
    # Bug fix: 使用清理过的 cleaned_state_dict 而不是原始的 common_state_dict
    try:
        preprocessed_common_state_dict = copy.deepcopy(cleaned_state_dict)
    except (TypeError, NotImplementedError) as e:
        # 如果 deepcopy 仍然失败，使用更保守的方法
        preprocessed_common_state_dict = {}
        for key, value in cleaned_state_dict.items():
            if key == 'args':
                if hasattr(value, '__dict__'):
                    preprocessed_common_state_dict[key] = {k: v for k, v in vars(value).items() 
                                                          if 'iterator' not in k.lower() and 'dataloader' not in k.lower()}
                else:
                    preprocessed_common_state_dict[key] = value
            elif key == 'rerun_state_machine':
                # 如果 rerun_state_machine 仍然有问题，设置为 None
                preprocessed_common_state_dict[key] = None
            else:
                preprocessed_common_state_dict[key] = value
    
    # Convert args key of type namespace to dictionary
    if 'args' in preprocessed_common_state_dict and not isinstance(preprocessed_common_state_dict['args'], dict):
        preprocessed_common_state_dict['args'] = vars(preprocessed_common_state_dict['args'])
...
def pretrain(
    ...
    if args.do_valid:
        if getattr(args, 'benchmark_eval', False):
...
def get_model(model_provider_func, model_type=ModelType.encoder_or_decoder, wrap_with_ddp=True, config=None, pg_collection=None):
    ...
    # 谱初始化
    # Spectral norm init: normalize 2D params (except embedding/lm_head) by spectral norm, then scale.
    if getattr(args, 'init_spectral_norm_scale', None) is not None:
        from megatron.core.utils import apply_spectral_norm_init_to_model
        ...
        if hasattr(inner, 'decoder') and hasattr(inner.decoder, 'layers') and len(inner.decoder.layers) > 0:
            ...
            config = get_model_config(model[0])
            gated_linear_unit = getattr(config, 'gated_linear_unit', False)
            num_attention_heads = getattr(config, 'num_attention_heads', None)
            num_query_groups = getattr(config, 'num_query_groups', None)
            kv_channels = getattr(config, 'kv_channels', None)
            if all(x is not None for x in (num_attention_heads, num_query_groups, kv_channels)):
                q_per_group = num_attention_heads // num_query_groups * kv_channels
                k_per_group = kv_channels
                v_per_group = kv_channels
                total_per_group = q_per_group + k_per_group + v_per_group
            else:
            ...
            for name, param in layer0.named_parameters():
                ...
                if 'self_attention.linear_qkv.weight' in name:
                    out_dim, _ = w.shape
                    if total_per_group is not None and out_dim % total_per_group == 0:
                        num_qg = out_dim // total_per_group
                        # if getattr(args, 'pion_split_qkv_per_head', True):
                        #     for g in range(min(num_qg, 3)):  # 只打印前 3 个 group 示例
                        #         w_q_g = w[g * total_per_group : g * total_per_group + q_per_group]
                        #         w_k_g = w[g * total_per_group + q_per_group : g * total_per_group + q_per_group + k_per_group]
                        #         w_v_g = w[g * total_per_group + q_per_group + k_per_group : (g + 1) * total_per_group]
                        #         parts.append(f'Q_g{g}={torch.linalg.norm(w_q_g, 2).item():.6f}')
                        #         parts.append(f'K_g{g}={torch.linalg.norm(w_k_g, 2).item():.6f}')
                        #         parts.append(f'V_g{g}={torch.linalg.norm(w_v_g, 2).item():.6f}')
                        #     if num_qg > 3:
                        #         parts.append(f'...({num_qg} groups)')
                        # else:
                        w_q = torch.cat([w[g * total_per_group : g * total_per_group + q_per_group] for g in range(num_qg)],dim=0,)
                        w_k = torch.cat([w[g * total_per_group + q_per_group : g * total_per_group + q_per_group + k_per_group] for g in range(num_qg)],dim=0,)
                        w_v = torch.cat([w[g * total_per_group + q_per_group + k_per_group : (g + 1) * total_per_group] for g in range(num_qg)],dim=0,)
                        for qkv_name, chunk in [('Q', w_q), ('K', w_k), ('V', w_v)]:
                            # sn = torch.linalg.norm(chunk, 2).item()
                            sn = torch.linalg.norm(chunk, 'fro').item()
                            parts.append(f'{qkv_name}={sn:.6f}')
                    else:
                        third = out_dim // 3
                        for qkv_name, chunk in [
                            ('Q', w[:third]),
                            ('K', w[third : 2 * third]),
                            ('V', w[2 * third : 3 * third]),
                        ]:
                    ...
                    parts.append(f'{label}={sn:.6f}')
            if parts:
                # print_rank_0('[Spectral norm init] Layer0: ' + ', '.join(parts))
                values=[float(part.split('=')[1]) for part in parts]
                mean_val = sum(values) / len(values)
                print('F-norm Layer0: ' + ', '.join(parts) + f' | mean={mean_val:.6f}')
                # print('F-norm Layer0: ' + ', '.join(parts))
    #     # Print spectral norm of all 2D params in first Transformer block (Q,K,V, o_project, up_project, down_project, and any others).
    #     m = model[0]
    #     inner = getattr(m, 'module', m)
    #     if hasattr(inner, 'decoder') and hasattr(inner.decoder, 'layers') and len(inner.decoder.layers) > 0:
    #         layer0 = inner.decoder.layers[0]
    ...
    #                     num_qg = out_dim // total_per_group
    #                     w_q = torch.cat(
    #                         [w[g * total_per_group : g * total_per_group + q_per_group] for g in range(num_qg)],
    #                         dim=0,
    #                     )
    #                     w_k = torch.cat(
    #                         [w[g * total_per_group + q_per_group : g * total_per_group + q_per_group + k_per_group] for g in range(num_qg)],
    #                         dim=0,
    #                     )
    #                     w_v = torch.cat(
    #                         [w[g * total_per_group + q_per_group + k_per_group : (g + 1) * total_per_group] for g in range(num_qg)],
    #                         dim=0,
    #                     )
    #                     for qkv_name, chunk in [('Q', w_q), ('K', w_k), ('V', w_v)]:
    #                         sn = torch.linalg.norm(chunk, 2).item()
    ...
    #             print_rank_0('[Spectral norm init] Layer0: ' + ', '.join(parts))
    # else:
    #     print_rank_0('Not applying spectral norm init')
...
def get_megatron_optimizer_config(args: Any) -> OptimizerConfig:
    ...
    config = None
    if args.optimizer == 'adam' or 'muon' in args.optimizer or 'pion' in args.optimizer:
...
def setup_model_and_optimizer(
    ...
    if args.skip_train:
        ...
        if 'muon' not in config.optimizer and 'pion' not in config.optimizer:
        ...
        elif config.optimizer == 'pion_msign':
...
def training_log(
    ...
    if args.save is not None and grad_norm is not None and is_last_rank():
            ...
            with open(csv_path, 'a', newline='', encoding='utf-8') as f:
                csv_writer = csv.DictWriter(f, fieldnames=['iteration', 'grad_norm'])
                if not file_exists:
                    csv_writer.writeheader()
                csv_writer.writerow({'iteration': iteration, 'grad_norm': f'{grad_norm:.6E}'})
    if iteration % args.log_interval == 0 or is_first_iteration:
        ...
        if iteration > 0 and not is_first_iteration:
            # 计算总训练时间估计
            total_estimated_time = elapsed_time_per_iteration * args.train_iters
            # 计算已用时间
            elapsed_total_time = elapsed_time_per_iteration * iteration
            # 计算剩余时间
            remaining_time = elapsed_time_per_iteration * (args.train_iters - iteration)
            ...
            log_string += f' | estimated total time: {format_time(total_estimated_time)}'
            log_string += f' | remaining: {format_time(remaining_time)}'
            
            # 计算预计完成时间
            from datetime import timedelta
            estimated_completion = datetime.now() + timedelta(seconds=remaining_time)
            log_string += f' | ETA: {estimated_completion.strftime("%Y-%m-%d %H:%M:%S")}'
...
def train(
    ...
    while iteration < args.train_iters:
        # 若开启，在未执行任何 step 前保存 iteration 0（与正常保存同路径，保证 .metadata / common.pt）
        if (
            iteration == 0
            and getattr(args, 'save_initial_checkpoint', False)
            and args.save is not None
        ):
            print_rank_0('Saving initial checkpoint (iteration 0)...')
            save_checkpoint_and_time(
                0,
                model,
                optimizer,
        ...
        if args.eval_interval and iteration % args.eval_interval == 0 and args.do_valid:
            ...
            # Benchmark evaluation (downstream tasks)
            if getattr(args, 'benchmark_eval', False):
                ...
                if benchmark_interval is not None and benchmark_interval > 0:
...
def evaluate(
    ...
    #     # Bug fix: 在 full_validation 模式下，eval_iters 是样本数，需要转换为 iteration 数
    # if args.full_validation and eval_iters is not None:
    #     # eval_iters 是样本数，每个 iteration 处理 global_batch_size 个样本
    #     # 使用向下取整，确保不超过 DataLoader 能提供的 batch 数
    #     # 这样可以避免 StopIteration，同时评估所有完整的 batches
    #     eval_iters = eval_iters // args.global_batch_size
    ...
    with torch.no_grad():
        ...
        while iteration < eval_iters:
            ...
            try:
                ...
                # DataLoader 迭代完毕，提前结束评估
                # 这种情况不应该发生（因为我们已经用向下取整计算了 eval_iters）
                # 但为了安全，捕获异常并正常结束
                if verbose:
                    print_rank_0(f'Evaluation stopped early at iteration {iteration}/{eval_iters} (data exhausted)')
                break
    ...
    # 此处eval_iters向上取整，会导致报错
    # # Bug fix: 在 full_validation 模式下，eval_iters 是样本数，需要转换为 iteration 数
    # if args.full_validation and eval_iters is not None:
    #     # eval_iters 是样本数，需要除以 global_batch_size 得到 iteration 数
    #     if eval_iters > args.global_batch_size: 
    #         eval_iters = (eval_iters + args.global_batch_size - 1) // args.global_batch_size  # 向上取整
...
def evaluate_and_print_results(
    ...
    # Bug fix: 在 full_validation 模式下，每次评估时都重新从 valid_dataloaders 获取样本数
    # 确保 args.eval_iters 始终是正确的样本数，而不是可能被修改的值
    if args.full_validation and hasattr(args, 'valid_dataloaders') and args.valid_dataloaders is not None:
        if not args.multiple_validation_sets:
            if args.valid_dataloaders[0] is not None:
                args.eval_iters = len(args.valid_dataloaders[0])
        else:
            if args.valid_dataloaders[0] is not None:
                args.eval_iters = [len(dl) for dl in args.valid_dataloaders]
    ...
    #     print_rank_0(f"total eval samples: {eval_iters}")
    #     args.eval_iters = eval_iters[0] if not args.multiple_validation_sets else eval_iters
    if args.full_validation:
        assert len(eval_iters) == len(data_iterators)

        # with full validation we need to distribute eval_iters to all ranks
        # 注意：此时 eval_iters 是样本数，需要在 rank 0 上转换为 iteration 数后再广播
        if mpu.get_tensor_model_parallel_rank() == 0:
            # Bug fix: 在 full_validation 模式下，eval_iters 是样本数，需要转换为 iteration 数
            if not args.multiple_validation_sets:
                # eval_iters 是 [样本数]，需要转换为 [iteration数]
                eval_iters_converted = [eval_iters[0] // args.global_batch_size]
            else:
                # eval_iters 是 [样本数1, 样本数2, ...]，需要转换为 [iteration数1, iteration数2, ...]
                eval_iters_converted = [samples // args.global_batch_size for samples in eval_iters]
            eval_iters = torch.tensor(eval_iters_converted, dtype=torch.long, device='cuda')
        else:
            eval_iters = torch.tensor([0] * len(eval_iters), dtype=torch.long, device='cuda')
        torch.distributed.broadcast(eval_iters, 0)
        eval_iters = eval_iters.tolist()
        print_rank_0(f"total eval iterations: {eval_iters} (each iteration processes {args.global_batch_size} samples)")
    ...
    if is_last_rank():
        ...
        if save_dir:
            ...
            # CSV 文件路径
            csv_file_path = os.path.join(save_dir, 'validation_metrics.csv')
            ...
            with open(csv_file_path, 'a', newline='', encoding='utf-8') as csvfile:
                fieldnames = ['iteration', 'consumed_train_samples', 'loss_key', 'loss_value', 'ppl_value']
                ...
                for key in total_loss_dict:
                    ...
                    # ppl_value = math.exp(min(20, loss_value))
                    is_loss_key = ('loss' in key)
                    if is_loss_key:
                        ppl_value = math.exp(min(20, loss_value))
                    else:
                        ppl_value = None
                    
                    # 如果有多个 validation sets，添加后缀
                    loss_key = f'{key}{suffix}' if suffix else key
                    
                    writer.writerow({
                        'iteration': iteration,
                        'consumed_train_samples': args.consumed_train_samples,
                        'loss_key': loss_key,
                        'loss_value': f'{loss_value:.6E}',
                        'ppl_value': f'{ppl_value:.6E}'
                    })
...
def get_train_valid_test_num_samples():
    ...
    # Bug fix: 如果没有提供 test_data_path，test_samples 应该为 None
    if args.test_data_path is None or (isinstance(args.test_data_path, list) and len(args.test_data_path) == 0):
        test_samples = None
    else:
        test_samples = args.eval_iters * args.global_batch_size
...
def build_train_valid_test_data_iterators(build_train_valid_test_datasets_provider):
    ...
    if valid_dataloaders is not None:
        ...
        if args.full_validation:
            ...
            # Bug fix: 保存 valid_dataloaders 到 args，以便在 evaluate_and_print_results 中重新获取样本数
            args.valid_dataloaders = valid_dataloaders
        ...
        if args.multiple_validation_sets:
            ...
            # Bug fix: 在 full_validation 模式下，使用 cyclic 类型，确保可以重复迭代
            valid_dl_type = "cyclic" if args.full_validation else dl_type
