...
def init_method_kaiming_normal():
    """Kaiming normal init for linear weight matrices (PyTorch defaults, fan_in)."""
    return functools.partial(torch.nn.init.kaiming_normal_, mode='fan_in')

def _gather_full_weight_for_spectral_norm(param, w):
    """
    若 param 为 TP 分片，则 all-gather 为完整矩阵后返回；否则返回 w。
    用于对完整 Q/K/V 做 spectral norm 时先得到整块矩阵。
    """
    if not torch.distributed.is_initialized():
        return w
    tp_group = parallel_state.get_tensor_model_parallel_group()
    ...
    rank = tp_group.rank()
    if partition_dim == 0:
        # 按行分片: 每 rank (local_out, in_dim)，gather 成 (out_dim, in_dim)
        local_out, in_dim = w.shape
        out_dim = local_out * world_size
        tensor_list = [torch.empty(local_out, in_dim, dtype=w.dtype, device=w.device) for _ in range(world_size)]
        torch.distributed.all_gather(tensor_list, w.contiguous(), group=tp_group)
        full = torch.cat(tensor_list, dim=0)
        return full
    else:
        # 按列分片: 每 rank (out_dim, local_in)，gather 成 (out_dim, in_dim)
        out_dim, local_in = w.shape
        in_dim = local_in * world_size
        tensor_list = [torch.empty(out_dim, local_in, dtype=w.dtype, device=w.device) for _ in range(world_size)]
        torch.distributed.all_gather(tensor_list, w.contiguous(), group=tp_group)
        full = torch.cat(tensor_list, dim=1)
        return full


def _scatter_full_weight_after_spectral_norm(param, full_w):
    """将归一化后的完整矩阵按当前 rank 的分片写回 param.data。"""
    if not torch.distributed.is_initialized():
        param.data.copy_(full_w.to(param.dtype))
        return
    tp_group = parallel_state.get_tensor_model_parallel_group()
    if tp_group.size() <= 1:
        param.data.copy_(full_w.to(param.dtype))
    ...
    rank = tp_group.rank()
    if partition_dim == 0:
        local_out = full_w.shape[0] // world_size
        shard = full_w[rank * local_out : (rank + 1) * local_out]
        param.data.copy_(shard.to(param.dtype).contiguous())
    else:
        local_in = full_w.shape[1] // world_size
...
#     # 从第一个 model 取 config，用于 linear_qkv 的 Q/K/V 分割（GQA 时非三等分）
#     config = get_model_config(model_list[0]) if model_list else None
#     num_attention_heads = getattr(config, 'num_attention_heads', None) if config else None
#     num_query_groups = getattr(config, 'num_query_groups', None) if config else None
#     kv_channels = getattr(config, 'kv_channels', None) if config else None
...
#                 w = param.data.float()
#                 c+=1
#                 if 'linear_qkv' in name and name.endswith('.weight'):
...
#                     # Megatron linear_qkv 存储为按 group 交错 [G0_Q,K,V | G1_Q,K,V | ...]，与 convert_to_hf 一致。
#                     # 需按 group 取出纯 Q/K/V，分别做谱范数后再按 group 写回。
#                     if total_per_group is not None and out_dim % total_per_group == 0:
#                         if not torch.distributed.is_initialized() or torch.distributed.get_rank() == 0:
#                             if c == 1:
#                                 print('Using GQA per-group layout for spectral norm (Q,K,V from each group then concat)')
#                         num_query_groups = out_dim // total_per_group
#                         # 按 group 提取纯 Q：每组取 [g*total : g*total+q_per_group]，再 concat
#                         w_q = torch.cat(
#                             [
#                                 full_w[g * total_per_group : g * total_per_group + q_per_group]
...
#                                     g * total_per_group
#                                     + q_per_group
#                                     + k_per_group : (g + 1) * total_per_group
#                                 ]
#                                 for g in range(num_query_groups)
#                             ],
#                             dim=0,
#                         )
#                         for block in (w_q, w_k, w_v):
#                             sn = torch.linalg.norm(block, 2).clamp(min=1e-12)
#                             block.div_(sn).mul_(scale)
#                         # 按 group 写回
#                         for g in range(num_query_groups):
#                             full_w[
#                                 g * total_per_group : g * total_per_group + q_per_group
#                             ].copy_(w_q[g * q_per_group : (g + 1) * q_per_group])
#                             full_w[
#                                 g * total_per_group
#                                 + q_per_group : g * total_per_group
#                                 + q_per_group
#                                 + k_per_group
#                             ].copy_(w_k[g * k_per_group : (g + 1) * k_per_group])
#                             full_w[
#                                 g * total_per_group
#                                 + q_per_group
#                                 + k_per_group : (g + 1) * total_per_group
#                             ].copy_(w_v[g * v_per_group : (g + 1) * v_per_group])
#                     else:
#                         # fallback：无 config 或 out_dim 不能整除时按连续块 [all Q|all K|all V] 或三等分
#                         if total_per_group is not None:
#                             q_rows = (out_dim // total_per_group) * q_per_group
#                             k_rows = (out_dim // total_per_group) * k_per_group
#                             v_rows = (out_dim // total_per_group) * v_per_group
#                             w_q = full_w[:q_rows]
#                             w_k = full_w[q_rows : q_rows + k_rows]
#                             w_v = full_w[q_rows + k_rows : q_rows + k_rows + v_rows]
#                         else:
#                             third = out_dim // 3
#                             w_q = full_w[:third]
#                             w_k = full_w[third : 2 * third]
#                             w_v = full_w[2 * third : 3 * third]
#                         for block in (w_q, w_k, w_v):
#                             sn = torch.linalg.norm(block, 2).clamp(min=1e-12)
#                             block.div_(sn).mul_(scale)
#                     _scatter_full_weight_after_spectral_norm(param, full_w)
#                 else:
#                     sn = torch.linalg.norm(w, 2).clamp(min=1e-12)
#                     param.data.copy_((w / sn * scale).to(param.dtype))

# def apply_spectral_norm_init_to_model(model_list, scale: float = 1.0, split_gate: bool = True):
def apply_spectral_norm_init_to_model(model_list, scale: float = 1.0, split_gate: bool = True, split_qkv_per_head: bool = True):
    """
    对模型中所有二阶参数（排除 embedding / output）做 spectral norm 归一化后乘 scale。
    对 linear_qkv：先按 TP 聚合成完整矩阵，再按 GQA/MHA 布局对 Q、K、V 分别做 spectral norm
    （与 Pion/Muon 的 qkv_split_shapes 一致：num_heads/num_query_groups*kv_channels, kv_channels, kv_channels 每组）。
    ...
    config = get_model_config(model_list[0]) if model_list else None
    gated_linear_unit = getattr(config, 'gated_linear_unit', False) if config else False
    ...
    print('We only mul the weights, no sn.')
    c = 0
    for model_module in model_list:
        for name, param in model_module.named_parameters():
            if param.dim() != 2:
                ...
                if 'linear_qkv' in name and name.endswith('.weight'):
                    ...
                    if not torch.distributed.is_initialized() or torch.distributed.get_rank() == 0:
                        if c == 1:
                            print(f'out_dim: {out_dim}, in_dim: {in_dim}')
                    if total_per_group is not None and out_dim % total_per_group == 0:
                        num_query_groups_dim = out_dim // total_per_group
                        # if split_qkv_per_head:
                        #     # Per-head (per group): each group's Q, K, V get their own SN, then scale
                        #     for g in range(num_query_groups_dim):
                        #         w_q_g = full_w[g * total_per_group : g * total_per_group + q_per_group]
                        #         w_k_g = full_w[g * total_per_group + q_per_group : g * total_per_group + q_per_group + k_per_group]
                        #         w_v_g = full_w[g * total_per_group + q_per_group + k_per_group : (g + 1) * total_per_group]
                        #         for block in (w_q_g, w_k_g, w_v_g):
                        #             sn = torch.linalg.norm(block, 2).clamp(min=1e-12)
                        #             block.div_(sn).mul_(scale)
                        # else:
                        if not torch.distributed.is_initialized() or torch.distributed.get_rank() == 0:
                            if c == 1:
                                print('Using GQA per-group layout for spectral norm (Q,K,V from each group then concat)')
                        w_q = torch.cat([full_w[g * total_per_group : g * total_per_group + q_per_group]for g in range(num_query_groups_dim)],dim=0,)
                        w_k = torch.cat([full_w[g * total_per_group+ q_per_group : g * total_per_group+ q_per_group+ k_per_group] for g in range(num_query_groups_dim)],dim=0,)
                        w_v = torch.cat([full_w[g * total_per_group+ q_per_group+ k_per_group : (g + 1) * total_per_group] for g in range(num_query_groups_dim)],dim=0,)
                        for block in (w_q, w_k, w_v):
                            sn = torch.linalg.norm(block, 2).clamp(min=1e-12)
                            block.div_(sn).mul_(scale)
                            # block.mul_(scale)
                        for g in range(num_query_groups_dim):
                            full_w[g * total_per_group : g * total_per_group + q_per_group].copy_(w_q[g * q_per_group : (g + 1) * q_per_group])
                            full_w[g * total_per_group+ q_per_group : g * total_per_group+ q_per_group+ k_per_group].copy_(w_k[g * k_per_group : (g + 1) * k_per_group])
                            full_w[g * total_per_group+ q_per_group+ k_per_group : (g + 1) * total_per_group].copy_(w_v[g * v_per_group : (g + 1) * v_per_group])
                    else:
                        if total_per_group is not None:
                            q_rows = (out_dim // total_per_group) * q_per_group
                            k_rows = (out_dim // total_per_group) * k_per_group
                    ...
                    sn = torch.linalg.norm(w, 2).clamp(min=1e-12)
                    param.data.copy_((w / sn * scale).to(param.dtype))
                    # param.data.copy_((w * scale).to(param.dtype))
