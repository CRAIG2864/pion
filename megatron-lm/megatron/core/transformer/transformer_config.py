...
class TransformerConfig(ModelParallelConfig):
    ...
    """Which norm to use for normalization layers, valid options are `LayerNorm`, `RMSNorm`, `ScaleOnly`, and `NoNorm`."""
    ...
    use_same_init_for_output_layers: bool = False
    """If True, O (linear_proj) and down (linear_fc2) use the same init as other layers
    (init_method), i.e. same std; otherwise they use output_layer_init_method with
    smaller std (scaled by 1/sqrt(2*num_layers))."""
    ...
    def __post_init__(self):
        ...
        if self.output_layer_init_method is None:
            if getattr(self, 'use_same_init_for_output_layers', False):
